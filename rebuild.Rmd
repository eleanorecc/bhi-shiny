...
source(here("R", "rebuilding.R"))

assess_year <- 2019
bhi_version <- "v2019"
scenario_folder <- "baltic"
...
tbl(bhi_db_con, "basins") %>% 
  collect() %>% 
  write_csv(here("data", "basins.csv"))

tbl(bhi_db_con, "regions") %>% 
  collect() %>% 
  write_csv(here("data", "regions.csv"))

## plotting configuration
tbl(bhi_db_con, "plot_conf") %>% 
  collect() %>% 
  write_csv(here("data", "plot_conf.csv"))
```

```{r extract relevant data from bhiprep github repo and write to data folder}
## assessment scores
read_csv(paste0(gh_raw_bhiprep, "scores.csv")) %>% 
  write_csv(here("data", "scores.csv"))
...
writeLines(bhi_theme, here("R", "theme.R"))
...
## regions shapefile:
if(!file.exists(here("data", "regions.rds"))){
...
## subbasins shapefile:
if(!file.exists(here("data", "subbasins.rds"))){
  ...
  sf::as_Spatial(subbasins_shp_simp) %>% 
    ...
    saveRDS(here("data", "subbasins.rds"))
...
get_layers(gh_raw_bhiprep, scenario_layers, 2014) %>% 
  write_csv(here("data", "layers_data.csv"))
```
...
source(here::here("R", "flowerplot.R"))
source(here::here("R", "theme.R"))

flower_data <- readr::read_csv(here("data", "scores.csv"))
fp_weights <- readr::read_csv(here("data", "layers_data.csv")) %>% 
