...
tbl(bhi_db_con, "basins") %>% 
  collect() %>% 
  write_csv(file.path(dir_main, "data", "basins.csv"))

tbl(bhi_db_con, "regions") %>% 
  collect() %>% 
  write_csv(file.path(dir_main, "data", "regions.csv"))

## plotting configuration
tbl(bhi_db_con, "plot_conf") %>% 
  collect() %>% 
  write_csv(file.path(dir_main, "data", "plot_conf.csv"))
```

```{r extract scores data from bhiprep github repo and write to data folder}
## assessment raw datasets, metadata
source(here::here("rebuild", "metainfotables.R"))
data_info <- make_data_table("https://raw.githubusercontent.com/OHI-Science/bhi-prep/master/", bhi_version)
...
bhiscores <- read_csv(paste0(gh_raw_bhi, scenario_folder, "/scores.csv"), col_types = cols()) %>% 
  left_join(
    ...
    # read_csv(file.path(dir_main, "data", "regions.csv"), col_types = cols()) %>% 
...
basinscores <- bhiscores %>% 
  ...
  left_join(
    read_csv(file.path(dir_main, "data", "basins.csv"), col_types = cols()) %>% 
...
rbind(select(bhiscores, goal, dimension, region_id, score), balticscores, basinscores) %>% 
  ...
  write_csv(file.path(dir_main, "data", "scores.csv"))
```
...
writeLines(bhi_theme, file.path(dir_main, "R", "theme.R"))
...
if(!file.exists(file.path(dir_main, "data", "regions.rds"))){
  ...
  #   sf::st_as_sf() %>% 
  #   write_rds(file.path(dir_main, "data", "regions.rds"))
...
if(!file.exists(file.path(dir_main, "data", "subbasins.rds"))){
  ...
  #   write_rds(file.path(dir_main, "data", "subbasins.rds"))
...
get_layers(gh_raw_bhiprep, scenario_layers, 2014) %>% 
  write_csv(file.path(dir_main, "data", "layers_data.csv"))
```
...
fp_weights <- readr::read_csv(file.path(dir_main, "data", "layers_data.csv")) %>% 
