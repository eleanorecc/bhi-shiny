...
flowerplotCardUI <- function(id){
  ...
  tagList(
    list(
      ...
      br(),
      imageOutput(ns("flowerplot"))
    )
  )
}

## flowerplot card server function ----
flowerplotCard <- function(input, output, session, flower_rgn_selected, flowerwidth){
  
  rgn_id <- flower_rgn_selected()
  rgn_id <- str_pad(rgn_id, 3, "left", 0)
  
  ## note flowerplots are created using function in rebuild/rebuild.R...
