...
get_layers_data <- function(gh_raw_bhi, layers, default_year){
  ...
  for(lyr in layers){
    ...
    ## read from layers or intermediate folder on github ----
    if(RCurl::url.exists(paste0(gh_raw_bhi, "layers/", unlist(lyr),  ".csv"))){
      ...
      message(sprintf("file %s doesn't exist at location", lyr))
    }
